...
for (const [id, product] of Object.entries(products)) {
  for (const field of ['what', 'usage', 'benefitsTitle']) {
    assert.ok(product[field]?.trim(), `${id}: missing ${field}`);
  }
  assert.ok(!('summary' in product), `${id}: duplicated card summary leaked into JSON payload`);
  ...
  assert.ok(html.includes(catalog[id].summary), `${id}: missing updated card summary`);
}
...
const hostedRuntime = await readFile(new URL('../public/script-v2.min.js', import.meta.url), 'utf8');
assert.ok(hostedRuntime.length > 1000, 'Hosted minified runtime must be generated');
assert.ok(hostedRuntime.length < script.length, 'Hosted runtime should be smaller than the source script');
await assert.rejects(
  readFile(new URL('../public/script-v2.js', import.meta.url), 'utf8'),
  error => error?.code === 'ENOENT',
  'Hosted build must not keep an unminified public copy of script-v2.js'
);
console.log('34 fiches verified: consumer copy, presentations, public data separation, and single minified runtime source.');
